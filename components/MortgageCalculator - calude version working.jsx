...
const MortgageCalculator = () => {
  ...
  const calculateAppreciationValue = () => {
    return parseNumberInput(totalPropertyCost) * 
  ...
  const calculateCapitalGainsTax = () => {
    ...
    if (holdingPeriod > 2) {
      ...
      taxAmount = Math.max(0, gains * 0.20);
    } else {
      // Short Term Capital Gains (STCG)
      gains = futureValue - propCost;
      taxAmount = Math.max(0, gains * 0.30);
    }
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => {
                        ...
                        if (newValue) {
                          ...
                          if (numValue > (parseNumberInput(totalPropertyCost) - parseNumberInput(downPayment))) {
                            alert('Loan amount cannot exceed (Property Cost - Down Payment)');
                            setLoanAmount(parseNumberInput(totalPropertyCost) - parseNumberInput(downPayment));
                          } else {
                  ...
                  <p className="text-sm text-gray-500">
                    ...
                    ({((parseNumberInput(loanAmount)/parseNumberInput(totalPropertyCost)) * 100).toFixed(1)}% of property value)
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input 
                      ...
                      min="1"
                      max="30"
                      onChange={(e) => setLoanTenure(Math.min(30, Math.max(1, Number(e.target.value))))}
                      className="pr-12"
                    />
              ...
              <div className="mt-4">
                <h3 className="text-lg font-semibold">
                  Monthly EMI: {formatIndianCurrency(calculateEMI(loanAmount, interestRate, loanTenure))}
                </h3>
              </div>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={loanSchedule}>
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="balance" stroke="#8884d8" name="Outstanding Balance" />
                    <Line type="monotone" dataKey="totalPayment" stroke="#82ca9d" name="Monthly Payment" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
